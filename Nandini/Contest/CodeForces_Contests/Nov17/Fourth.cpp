...
void solve() 
{
    ...
    {
        cin >> c >> d;
        hurdles.push_back({c,d}); // start,jump req-> d-c+1
    }
    vector<pll> powerup;
    for(int i=0 ; i<m ; i++)
    {
        cin >> c >> d;
        powerup.push_back({c,d}); // start,jump
    }

    ll ans =0,power=1,j=0;
    priority_queue<ll> pq;

    for (int i = 0; i < n; i++) {
        ll start = hurdles[i].first;
        ll jump = hurdles[i].second;
        
        while(j<m && powerup[j].f<start){
            pq.push(powerup[j++].s);
        }
